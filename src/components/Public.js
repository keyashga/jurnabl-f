...
const Public = () => {
    ...
    const [consistency, setConsistency] = useState(0); // New state for consistency
    ...
    // Calculate consistency on the frontend
    useEffect(() => {
        if (user && user.createdAt && user.journalCount !== undefined) {
            const today = new Date();
            const createdDate = new Date(user.createdAt);

            const diffTime = Math.abs(today.getTime() - createdDate.getTime());
            const diffDays = Math.ceil(diffTime / (1000 * 60 * 60 * 24)); // At least 1 day

            let calculatedConsistency = 0;
            if (diffDays > 0) {
                calculatedConsistency = (user.journalCount / diffDays) * 100;
                // Optional: Cap at 100% or adjust as per your definition of "100% consistent"
                // calculatedConsistency = Math.min(calculatedConsistency, 100);
            }

            setConsistency(parseFloat(calculatedConsistency.toFixed(2)));
        } else {
            setConsistency(0); // Reset if user data is not yet available or incomplete
        }
    }, [user]); // Recalculate when user object changes (which includes createdAt and journalCount)
                    ...
                    <div className="profile-right">
                        ...
                        <div className="user-stats-horizontal">
                            ...
                            <div className="stat-item-horizontal">
                                ...
                                <div className="stat-content">
                                    <Text className="stat-number">
                                        {consistency}% {/* Use the calculated consistency state */}
                                    </Text>
